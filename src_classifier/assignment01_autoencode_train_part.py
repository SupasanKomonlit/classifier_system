...
# ref04 : https://blog.keras.io/building-autoencoders-in-keras.html
...
# Part Start Setup Model for encoder
    model = Sequential()
    model.name = "auto_encoder"
    # Add all layers
    ## Add Convolution layer for encode
    ...
    model.add( Conv2D( 64 ,
            ...
            padding='same',
            activation='relu') )
#    ## Add Part pooling for pool feature
    model.add( MaxPooling2D( (2,2),
            padding='same',
            data_format='channels_last') )
    ## Add Layer End part encoder
    model.add( Conv2D( 32 ,
            (3,3),
            padding='same',
            activation='relu') )
    ...
    ## Start layer convolution deocnde
    model.add( Conv2D( 64 , 
            (3,3),
            padding='same',
            activation='relu' ) )
    model.add( Conv2D( 128 , 
            ...
            padding='same',
            activation='relu' ) )
    ## Add Seventh Layer is Convolution to make same input follow pattern autoencode
    model.add( Conv2D( 3 if color else 1 ,
            (3,3),
            activation='sigmoid',
            padding='same') )
...
# Part Start Train Model
    ...
    # Deactivate tune mode autoencode part
    for layer in model.layers:
        layer.trainable = False 

# Part Start about Fully connected
